...
export class CrossEntityConflictHistoryService {
  ...
  private _buildRobotEntries(
    ...
    // For robot card: show one entry for the robot itself + one entry per unique wiring value
    const entryMap = new Map<string, CrossEntityHistoryEntry>();

    // Robot's own entry (from first conflict's robotValue)
    ...
    // Wiring entries — one per unique (wiringId, wiringValue) pair
  ...
  private _buildWiringEntries(
    ...
    // For wiring card: show one entry for the wiring itself + one entry per unique robot value
    const entryMap = new Map<string, CrossEntityHistoryEntry>();

    // Wiring's own entry (from first conflict's wiringValue)
    const first = conflicts[0];
    ...
    // Robot entries — one per unique (robotId, robotValue) pair
    conflicts.forEach((conflict) => {
