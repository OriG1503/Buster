...
export class FictiveDataTransferService {
  ...
  public async clearFkFields(service: EntityService<{ id: string }>, fictive: BaseEntity): Promise<void> {
    ...
    const fkFieldsToClear = Object.keys(fictiveRecord).filter(
      (k) => k.endsWith('Id') && k !== 'id' && fictiveRecord[k] != null,
    );
    if (!fkFieldsToClear.length) {
      return;
    }
    ...
    await service.update(fictive.id, clearFields, {}, fictive.source, {}, fictive.notes, {}, fictive.sourceTime);
  }
