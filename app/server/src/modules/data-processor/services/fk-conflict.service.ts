...
export class FkConflictService {
  ...
  public async detectTwoFathersOnInsert(
    ...
    const results = await Promise.all(
      Object.keys(fields)
        ...
        .map((fkField) => this._checkOneFkTwoFathersOnInsert(service, id, fkField, fields, source, notes, username)),
    );
  ...
  public async detectTwoFathersOnGapFill(
    ...
    const results = await Promise.all(
      Object.keys(fieldsToUpdate)
        ...
        .map((fkField) => this._checkOneFkTwoFathersOnGapFill(service, id, fkField, fieldsToUpdate, source, notes, username)),
    );
  ...
  public async detectTwoChilds(
    ...
    const results = await Promise.all(
      Object.keys(fields)
        ...
        .map((fkField) => this._detectOneTwoChildsConflict(service, id, fkField, stored, storedRecord, fields, source, notes, username)),
    );
  ...
  private async _detectOneTwoChildsConflict(
    ...
    return this._relationalConflictService.detectTwoChilds(
      ...
      stored.source?.[fkField] ?? null, stored.notes?.[fkField] ?? null,
