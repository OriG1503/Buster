...
export class ProcessReportService {
  ...
  private _buildFlyingCellMap(flyingFields: FlyingField[]): FlyingCellMap {
    ...
    flyingFields.forEach(({ entity, rowIndex, isUuidRed }) => {
      ...
      if (uuidCsvHeader) { map.get(excelRow)!.redCols.add(uuidCsvHeader); }
    });
    return map;
  }
  ...
  private _applyMissingUuidHighlights(sheet: ExcelJS.Worksheet, headerColMap: Map<string, number>): void {
    ...
    sheet.eachRow((row, rowNumber) => {
      ...
      Object.values(configs).forEach((entityConfig) => {
        ...
        if (!uuidColIndex) { return; }
        if (row.getCell(uuidColIndex).value) { return; }

        const dataColumns = Object.values(columns).filter((col) => col.valueType === 'string' || col.valueType === 'bool');
        dataColumns.forEach((col) => {
          const colIndex = headerColMap.get(col.csvHeader);
          if (!colIndex) { return; }
          const cell = row.getCell(colIndex);
          if (cell.value !== null && cell.value !== '') { cell.fill = YELLOW_FILL; }
        });
      });
    });
  }
  ...
  private _applyUnknownColumnHighlights(sheet: ExcelJS.Worksheet, unknownColumns: string[], headerColMap: Map<string, number>): void {
    unknownColumns.forEach((colName) => {
      const colIndex = headerColMap.get(colName);
      if (colIndex === undefined) { return; }
      sheet.getColumn(colIndex).eachCell((cell) => { cell.fill = ORANGE_FILL; });
    });
  }
}
